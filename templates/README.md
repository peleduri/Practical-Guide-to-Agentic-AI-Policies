...
The **five first controls** in [`../start-here.md`](../start-here.md) each ship a working
artifact here — discovery, the managed baseline, the enforcement hook, scoped identity, and
the kill switch. That is the line between this and a risk catalog: a framework tells you to
*"discover shadow agents"* or *"pre-build a kill switch"*; these files are the discovery scan
and the kill switch.
...
|------|-----------|------|
| `discovery/inventory-agents.sh` | Read-only scan: installed agents + the MCP servers they reach (control #1, "discover before you defend") | [1](../wiki/part-1-risk-surface-and-control-model.md) |
...
| `incident/agent-kill-switch.sh` | Fail-safe kill switch: snapshot evidence, kill, freeze, cut egress, revoke (control #5) | [9](../wiki/part-9-detection-monitoring-ir.md) |
| `headless/permission-gate.sh` | Deny-by-default gate for headless agent-driving; "ask" collapses to deny, fails closed | [14](../wiki/part-14-multi-agent-a2a.md) |
...
- **Discovery** — run `discovery/inventory-agents.sh` fleet-wide (per user, via MDM), ship the
  JSON Lines to your SIEM, and diff against your sanctioned allowlist. See `discovery/README.md`.
...
- **Kill switch** — `incident/agent-kill-switch.sh` runs as a dry run by default; rehearse it
  cold, wire `cut_egress()` to your isolation control, and pre-stage the revoke checklist. See
  `incident/README.md`.
- **Headless gate** — register `headless/permission-gate.sh` as the driven agent's PreToolUse
  hook for any headless / agent-driven run (acpx, CI, orchestrator). It denies by default,
  collapses "ask" to deny, and fails closed. See `headless/README.md`.
